    ...
    <div className="p-6 space-y-8">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 px-10">
        <div className="bg-white p-4 rounded shadow">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={dashboardData.patientTotals} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
              <XAxis dataKey="month" />
              <YAxis />
              <Tooltip />
        ...
        <div className="bg-white p-4 rounded shadow">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={dashboardData.patientsIn} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
              <XAxis dataKey="day" />
              <YAxis />
              <Tooltip />
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 px-10 gap-6">
          ...
          <ul className="space-y-2 text-sm">
            {Array.from({ length: 5 }).map((_, i) => (
              <li key={i} className="flex justify-between items-center border-b pb-1">
                ...
                <button className="text-blue-600 border border-blue-600 px-2 py-1 rounded hover:bg-blue-600 hover:text-white text-xs">Take Up</button>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="bg-white p-4 rounded shadow">
          ...
          <ul className="space-y-2 text-sm">
            {['John Kitili', 'Jane Smith', 'Richard Marx', 'Dr. Benjamin Mkapa'].map((doc, i) => (
              <li key={i} className="flex justify-between border-b pb-1">{doc}</li>
            ))}
          </ul>
        </div>
      </section>
